...
def test__cookiehandler__response_cookies__saved():
    ...
    wrapped_fetcher = MyOpener(MyResponse('my content',
                                             headers={'set-cookie': 'III_SESSION_ID=0bf0ce5fb384ddda97aff805246ec7a2; path=/, SESSION_SCOPE=0; path=/'}))
    fetcher = CookieHandler(wrapped_fetcher)
    assert 'my content' == fetcher('http://www.google.ca/').content
    assert fetcher.cookie_jar.has_key('III_SESSION_ID')
    assert fetcher.cookie_jar.has_key('SESSION_SCOPE')
